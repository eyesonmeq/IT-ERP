...
public class JWTRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		AuthenticationTokenDTO redisToken = redisTemplate.opsForValue()
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
		AuthenticationTokenDTO token = JwtUtil.parseJWT(String.valueOf(((AuthenticationTokenDTO) auth).getToken()));
		if (token == null) {
		...
		return new SimpleAuthenticationInfo(redisToken.getToken(), redisToken.getAuth(), this.getName());
	}
}
